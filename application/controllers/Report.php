		...
		if ($this->report_model->read()->num_rows() > 0) {
			foreach ($this->report_model->read()->result() as $report) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-success" onclick="edit('.$report->id.')">Edit</button> <button class="btn btn-sm btn-danger" onclick="remove('.$report->id.')">Delete</button>'
				);
			}
		} else {
			$data = array();
