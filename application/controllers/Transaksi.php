...
class Transaksi extends CI_Controller {
	...
	{
		// header('Content-type: application/json');
		if ($this->transaksi_model->bulan()->num_rows() > 0) {
			foreach ($this->transaksi_model->bulan()->result() as $transaksi) {
				$data[] = array(
	...
	public function monthly($year, $month) {
        $this->load->model('transaksi_model');
        $data['monthly_data'] = $this->transaksi_model->getMonthlyData($year, $month);
