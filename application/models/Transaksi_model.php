...
class Transaksi_model extends CI_Model {
	...
	{
		// $this->db->select('transaksi.id, transaksi.tanggal, transaksi.jumlah_uang');
		// $this->db->from($this->table);
		// $this->db->order_by('transaksi.tanggal');
		// return $this->db->get();

		return $this->db->query("SELECT transaksi.id,DATE(transaksi.tanggal) as tgl, sum(transaksi.jumlah_uang) as jumlah_uang FROM `transaksi` GROUP BY tgl  ");
	}
